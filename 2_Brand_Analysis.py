...
page_header("Brand Analysis", "Brand presence, ratings, top-10 ranking performance, and visibility score.")
...
c1.metric("Total Brands", df["brand"].nunique())
c2.metric("Top Brand", top_brand_by_count(df))
c3.metric("Highest Avg Visibility", df.groupby("brand")["visibility_score"].mean().idxmax())
c4.metric("Highest Avg Rating", df.groupby("brand")["rating"].mean().idxmax())

col1, col2 = st.columns(2)
with col1:
    st.plotly_chart(charts.brand_product_count(df), use_container_width=True)
with col2:
    st.plotly_chart(charts.brand_avg_rating(df), use_container_width=True)

col1, col2 = st.columns(2)
with col1:
    st.plotly_chart(charts.top_brands_top10(df), use_container_width=True)
with col2:
    st.plotly_chart(charts.visibility_by_brand(df), use_container_width=True)

st.subheader("Brand Summary Table")
st.dataframe(grouped_summary(df, "brand"), use_container_width=True, hide_index=True)
